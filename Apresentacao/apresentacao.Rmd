...
* Greve da Polícia Militar do Espírito Santo entre 04/02/2017 e 24/02/2017 (21 dias de duração)
    ...
    + Reivindicações:
        - Reajuste de salários - foram 4 anos sem reajustes com um período de IPCA acumulado em 29,28% entre 2013 e 2016
...
> Estimar o efeito impacto da Greve da PM na taxa de homicídios do Estado - Quanto da variação na taxa de homicídios é atribuível ao evento?

* Estatísticas de homicídios são comumente reportadas em "número de homicídios/100 mil habitantes/ano". Este é o número que vamos reportar e analisar.
...
Os dados de Homicídios são do Sistema de Informações de Mortalidade (SIM) reportados pelo SUS. Esses dados são retirados das declarações de óbito, preenchidos por médicos que fornecem, entre outras informações, o município e a causa da morte. 


Uso somente os dados de homicídios, sem ser possível diferenciar entre culposo e doloso, e agrego ao nível Estado.
...
df_ajuda1<-df_pop %>% group_by(Onde) %>%
    summarise(Populacao_proj_2017 = sum(Populacao_proj_2017))
df_ajuda1$Pop_100mil<-df_ajuda1$Populacao_proj_2017/100000
...
* A ideia básica é que, sob as hipóteses de identificação, ao estimarmos o mapeamento entre o grupo de tratados e o grupo de controle no período pré-intervenção, teremos um contrafactual válido para o período de tratamento
* A partir de um modelo de fatores comuns, sob as hipóteses de que o grupos de controle não sofre tratamento e que os grupos são compatíveis, demonstra-se a normalidade assintótica do efeito médio de tratamento para um mapeamento genérico
    + Também é possível discriminar o efeito por dia
* O mapeamento escolhido para este caso foi o Random Forest
...
* A ideia aqui é que, ao construirmos a combinação convexa do grupo de controle que mais se aproxime do grupo tratado em todas as características observáveis correlacionadas com a variável de resposta do tratamento, podemos contruir um contrafactual para o grupo tratado utilizando as variáveis de resposta dos controles no período de intervenção
...
Para passarmos para o efeito em número de homicídios, precisamos multiplicar esses resultados pela população do ES em centenas de milhares (39.25341) para termos o número de homicídios médios por dia de greve e depois pela duração da greve, 
duração da greve (21 dias) para termos o efeito do evento no número de homicídios 

## Análise dos Efeitos Médios de Tratamento
* ArCo:

$$
(0.1518 * 39.25341) * (21) = (5.9585) * (21) = 125.1285
$$
...
\textit{5.79 homicídios a mais por dia e 121.6 homicídios a mais no total.}
